...
interface HandleApiErrorResult extends ErrorResponse {
  status: number;
}

export function handleApiError(error: unknown): HandleApiErrorResult {
  // error is possibly any type, so we need to safely access its properties
  const err = error as Partial<PrismaError> & Partial<Error>;
