...
const EmptySpace: React.FC<EmptySpaceProps> = ({
  ...
  <>
    <div className="max-w-lg">
      {Icon ? (
        <div className="mb-4">
          <Icon className="h-14 w-14 text-secondary-foreground" />
        </div>
      ) : null}

      <h2 className="text-lg font-medium text-custom-text-100">{title}</h2>
      <div className="mt-1 text-sm text-secondary-foreground">{description}</div>
      <ul
        role="list"
        className="mt-6 divide-y divide-custom-border-200 border-b border-t border-custom-border-200"
      >
        {children}
      </ul>
      {link ? (
  ...
  let spaceItem = (
    ...
    >
      <div className="flex-shrink-0">
        <span className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-current">
          <Icon className="h-6 w-6 text-secondary" aria-hidden="true" />
        </span>
      </div>
      <div className="min-w-0 flex-1 text-secondary-foreground">
        <div className="text-sm font-medium group-hover:text-custom-text-100">
          {title}
        </div>
      ...
      <div className="flex-shrink-0 self-center">
        <ChevronRight
          className="h-5 w-5 text-secondary-foreground group-hover:text-custom-text-100"
          aria-hidden="true"
        />
      </div>
    </div>
