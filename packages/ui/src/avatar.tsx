...
export function getUserAvatarUrl(user?: User | null) {
  ...
  const ogAvatar = `https://api.dub.co/og/avatar?seed=${encodeURIComponent(
    user.id
  )}`;
  const encodedOGAvatar = `https://api.dub.co/og/avatar${encodeURIComponent(
  ...
  return user.email
    ? `https://www.gravatar.com/avatar/${sha256(
        user.email
      )}?d=${encodeURIComponent(encodedOGAvatar)}`
    : ogAvatar;
}
